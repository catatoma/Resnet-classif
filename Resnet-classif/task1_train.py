...
df = pd.read_csv("data_aait/Task1/task1/train_data/annotations.csv")
...
dataset = ImageDataset(
    root_dir='data_aait/Task1/',
    csv_file='data_aait/Task1/task1/train_data/annotations.csv',
    transform = augment()
)
...
def plot_and_save_metrics(train_loss, train_accuracy, val_loss, val_accuracy, fold, save_path):
    ...
    plt.savefig(os.path.join(save_path, f'training_metrics_fold_{fold+1}.png'))
    plt.close()
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
    ...
    plot_and_save_metrics(train_loss_all, train_accuracy_all, val_loss_all, val_accuracy_all, fold, 'results_aait/frozen_unfrozen_t1')
    ...
    torch.save(model.state_dict(), os.path.join(model_save_path, f'model_frozen_unfrozen_{fold}_t1.pth'))
